    ...
    <>
      <header
        className={`fixed top-0 left-0 w-full z-40 transition-all duration-300 ${
          scrolled
            ? "py-3 bg-nbdark/80 backdrop-blur-md"
            : "py-6 bg-transparent"
        }`}
      >
        <div className="container mx-auto px-4 md:px-8 flex justify-between items-center">
          <Link to="/" className="cursor-pointer">
            <h1 className="flex items-center gap-2 text-xl md:text-2xl font-bold font-display">
              <img
                src="/lovable-uploads/NB media2-04.png"
                alt="NB Media Logo"
          ...
          <nav className="hidden md:flex items-center space-x-8">
            <Link
              ...
              className={`transition-colors duration-300 cursor-hover ${
                isActive("/")
                  ? "text-nborange"
                  : "text-white hover:text-nborange"
              }`}
            >
              Home
            </Link>
